...
$appList = [
	['name' => 'QRShop', 'color' => 'red'],
	['name' => 'Notes', 'color' => 'white'],
	['name' => 'SmartCity', 'color' => 'Yellow'],
];
...
$sheet->setCellValueByColumnAndRow($columnPosition, $startLine, 'Our apps');
...
// Формируем список
foreach ($appList as $key=>$catItem) {
	// Перекидываем указатель на следующую строку
    $startLine++;
    // Указатель на первый столбец
    $currentColumn = $columnPosition;
